  ...
  return <>
    <Screen scrollable contentContainerStyle={styles.screen}>
      ...
      {desktopInfo ? <Card style={styles.about}>
        ...
        {desktopUpdate.state.phase === 'downloaded' ? <Button label="Restart DOIT AI" icon="refresh" onPress={desktopUpdate.install} /> : ['available', 'downloading'].includes(desktopUpdate.state.phase) ? <Button label={desktopUpdate.state.phase === 'downloading' ? `Downloading automatically · ${desktopUpdate.state.percent ?? 0}%` : 'Preparing automatic download…'} icon="cloud-download-outline" disabled onPress={() => undefined} /> : <Button label="Check for updates" variant="secondary" icon="refresh-outline" disabled={desktopUpdate.state.phase === 'checking'} onPress={desktopUpdate.check} />}
