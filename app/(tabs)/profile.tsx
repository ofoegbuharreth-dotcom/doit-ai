...
export default function ProfileScreen() {
  ...
  const backup = { goals, milestones, tasks, activity, checkIns, progressEntries, focusSessions };
  const runExport = async (kind: 'backup' | 'csv') => { setExporting(true); setExportMessage(''); try { if (kind === 'backup') await exportWorkspaceBackup(backup); else await exportProgressCsv(backup); setExportMessage(kind === 'backup' ? 'Backup ready to save or share.' : 'Progress export ready.'); } catch (value) { setExportMessage(value instanceof Error ? value.message : 'Could not export your data.'); } finally { setExporting(false); } };
