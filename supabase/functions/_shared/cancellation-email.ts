...
export async function sendCancellationOwnerEmail(payload: CancellationEmailPayload) {
  ...
  const feedbackTo = Deno.env.get('FEEDBACK_TO_EMAIL') ?? 'ofoegbuharreth@gmail.com';
  const feedbackFrom = Deno.env.get('FEEDBACK_FROM_EMAIL') ?? 'DOIT AI <onboarding@resend.dev>';
  const logoUrl = Deno.env.get('APP_LOGO_URL');
  if (!resendKey) throw new Error('Feedback email is not configured.');
  ...
  const sendEmail = (from: string, attempt: string) => fetch('https://api.resend.com/emails', {
    method: 'POST',
  ...
  return await email.json().catch(() => ({ accepted: true }));
}
